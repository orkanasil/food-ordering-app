<template>
    <div v-for="(element, index) in pizzaTypes" :key="index">
        <h2>{{ element.title }}</h2>
        <ListItem v-for="pizza in element.pizzas" :key="pizza.id" class="shadow-2xl" :pizza="pizza"
            @see-detail="seeDetail(pizza.id)" @increment="increment(pizza.id)" @decrement="decrement(pizza.id)"
